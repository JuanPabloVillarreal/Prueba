...
namespace TGC.MonoGame.TP
{
    ...
    public class TGCGame : Game
    {
        ...
        private Texture2D Comun { get; set; }
        ...
        protected override void LoadContent()
        {
            ...
            Model = Content.Load<Model>(ContentFolder3D + "tgc-logo/sphere");

            // Cargo un efecto basico propio declarado en el Content pipeline.
            // En el juego no pueden usar BasicEffect de MG, deben usar siempre efectos propios.
            Effect = Content.Load<Effect>(ContentFolderEffects + "BasicShader");

            Overlay = Content.Load<Texture2D>(ContentFolder3D + "fondo-blanco");
            ...
            foreach (var mesh in Model.Meshes)
                ...
                {
                    Comun = Content.Load<Texture2D>(ContentFolder3D + "overlay");
        ...
        protected override void Draw(GameTime gameTime)
        {
            ...
            foreach (var mesh in Model.Meshes)     /* --------> si lo dejo aca, aparece una pantalla volando*/
            {
               World = mesh.ParentBone.Transform * rotationMatrix;
                Effect.Parameters["World"].SetValue(World);
                Effect.Parameters["baseTexture"].SetValue(Comun);
               mesh.Draw();
            }
            GraphicsDevice.SetRenderTarget(SceneRenderTarget);
            Effect.Parameters["baseTexture"]?.SetValue(SceneRenderTarget);

            // SI BORRO LO ANTERIOR QUEDA RE LOCO
            #endregion
            #region Pass 2
